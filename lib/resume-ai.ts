...
export async function generateTailoredResume(jobAnalysis: JobAnalysis, resumeData: ResumeData) {
  ...
  `
      ...
      lastError = error
      console.error(`Resume generation attempt ${attempt} failed:`, error.message)
  ...
  throw new Error(
    `Failed to generate resume after ${maxRetries} attempts. Last error: ${lastError?.message || "Unknown error"}`,
  )
}
...
export async function generateTailoredCoverLetter(jobAnalysis: JobAnalysis, resume: TailoredResume): Promise<string> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error
      console.error(`Cover letter generation attempt ${attempt} failed:`, error.message)
  ...
  throw new Error(
    `Failed to generate cover letter after ${maxRetries} attempts. Last error: ${lastError?.message || "Unknown error"}`,
  )
}
